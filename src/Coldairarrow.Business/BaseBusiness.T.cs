...
namespace Coldairarrow.Business
{
    ...
    public abstract class BaseBusiness<T> where T : class, new()
    {
        ...
        public async Task<List<SelectOption>> GetOptionListAsync(OptionListInputDTO input, string textFiled, string valueField, IQueryable<T> source = null)
        {
            ...
            string where = " 1=1 AND Deleted = FALSE";
            var search = input.pageInput.Search;
            ...
            //自定义其他筛选条件
            var lqwhere = LinqHelper.True<T>();
            if (search != null && source.IsNullOrEmpty())
            {
                search?.ForEach(item =>
                {
                    if (!item.Condition.IsNullOrEmpty() && !item.Keyword.IsNullOrEmpty())
                    {
                        ...
                        lqwhere = lqwhere.AndIf(true, newWhere);
                    }
                });
            }

            List<T> newQList = await GetNewQ().Where(lqwhere).Where(where, ids, input.q).GetPageListAsync(input.pageInput);
