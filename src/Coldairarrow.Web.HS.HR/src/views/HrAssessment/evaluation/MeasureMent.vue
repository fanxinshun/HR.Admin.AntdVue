    ...
    <a-spin :spinning="loading">
      <a-form-model ref="ruleForm" :model="ruleForm" :rules="rules" v-bind="layout">
        ...
        <a-form-model-item label=" 测评对象">
          <a-button>{{ruleForm.user_name}}</a-button>
        </a-form-model-item>
        <template v-if="type === 'view'">
          ...
          <a-form-model-item label="下级测评人">
            <div class="table-operator"><a-button v-for="item in ruleForm.lowerEvaluation" :key="item">{{item}}</a-button></div>
          </a-form-model-item>
          <a-form-model-item label="HR部门负责人">
            <div class="table-operator"><a-button v-for="item in ruleForm.hrDepartment" :key="item">{{item}}</a-button></div>
          </a-form-model-item>
        </template>
        <br/>
        <a-table
...
<style type="text/css">
  ...
  .measureMentContent.content{margin: 0 auto;}
  .measureMentContent.content .table-operator{margin-bottom: 0}
</style>
