...
namespace Coldairarrow.Util
{
    public static class ImgVerifyCodeHelper
    {
        ...
        /// <returns>图片byte</returns>
        public static byte[] BuildVerifyCodeByte(int length)
        {
        ...
        /// <returns></returns>
        public static bool VerifyCode(this string verify_code)
        {
            var key = verify_code.ToUpper();
            if (key.IsNullOrEmpty() || !RedisHelper.Exists(key))
            {
                return false;
            }
            RedisHelper.Del(key);
        ...
        public class VerifyCodeFactory
        {
            ...
            {
                return CreateValidateGraphic(validateCode, 14, 40);
            }
            ...
            {
                Bitmap image = new Bitmap((int)Math.Ceiling(validateCode.Length * (fontsize + 6)), height);
