            ...
            <a-form-item>
              <a-row :gutter="16">
                ...
                <a-col class="gutter-row" :span="6">
                  <a-spin v-if="isVerification" />
                  <img :src="imgSrc" v-else alt="" @click="getVerification_code" style="display: block;width: 100%;height: 40px;">
                </a-col>
              </a-row>
            </a-form-item>
            <a-form-item>
    ...
    methods: {
      ...
      handleSubmit(e) {
        ...
        this.form.validateFields((errors, values) => {
          //校验成功
          if (!errors) {
            var values = Object.assign(this.form.getFieldsValue(), {
              projectId: TokenCache.getProject(),
              loginSource: 0
            }) // loginSource 0表示登陆人事测评系统
            this.loading = true
      ...
      getVerification_code() {
        ...
        const host = process.env.NODE_ENV === 'development' ? process.env.VUE_APP_LocalRootUrl : process.env.VUE_APP_PublishRootUrl
        this.imgSrc = host + '/Base_Manage/Home/VerifyCode?r=' + Math.random()
        if (this.imgSrc) {
